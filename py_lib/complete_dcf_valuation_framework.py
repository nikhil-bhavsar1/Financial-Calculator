def fcff(ebit: float, tax_rate: float, depreciation: float, capex: float, change_in_nwc: float) -> float:
    """
...
def fcff_from_net_income(net_income: float, interest_expense: float, tax_rate: float, depreciation: float, capex: float, change_in_nwc: float) -> float:
    """
    Free Cash Flow to Firm (FCFF) - from Net Income
    Formula: Net Income + Interest(1 - Tax Rate) + Depreciation - CapEx - Change in NWC
    """
    return net_income + interest_expense * (1 - tax_rate) + depreciation - capex - change_in_nwc
...
def net_debt(total_debt: float, cash_and_cash_equivalents: float) -> float:
    """
    Net Debt
...
def fcfe(net_income: float, capex: float, depreciation: float, change_in_nwc: float, new_debt: float, debt_repayment: float) -> float:
    """
