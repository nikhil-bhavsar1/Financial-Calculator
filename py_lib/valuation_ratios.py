...
def price_to_earnings_ratio(market_price_per_share: float, earnings_per_share_basic: float) -> float:
    """
    Price-to-Earnings (P/E) Ratio
    Formula: Market Price Per Share / Earnings Per Share (EPS)
    """
    return market_price_per_share / earnings_per_share_basic

def price_to_earnings_ratio_alt(market_capitalization: float, profit_for_the_year: float) -> float:
    """
    Price-to-Earnings (P/E) Ratio - Alternative
    Formula: Market Capitalization / Net Income
    """
    return market_capitalization / profit_for_the_year

def book_value_per_share(total_equity: float, number_of_shares: float) -> float:
    """
    Book Value Per Share
    Formula: Total Equity / Shares Outstanding
    """
    return total_equity / number_of_shares
...
def revenue_per_share(total_revenue: float, number_of_shares: float) -> float:
    """
    Revenue Per Share
    Formula: Total Revenue / Shares Outstanding
    """
    return total_revenue / number_of_shares
...
def price_to_sales_ratio_alt(market_capitalization: float, total_revenue: float) -> float:
    """
    Price-to-Sales (P/S) Ratio - Alternative
    Formula: Market Capitalization / Total Revenue
    """
    return market_capitalization / total_revenue

def operating_cash_flow_per_share(operating_cash_flow: float, number_of_shares: float) -> float:
    """
...
def price_to_cash_flow_ratio_alt(market_capitalization: float, operating_cash_flow: float) -> float:
    """
    Price-to-Cash Flow (P/CF) Ratio - Alternative
    Formula: Market Capitalization / Operating Cash Flow
    """
    return market_capitalization / operating_cash_flow
...
def earnings_to_price_ratio(earnings_per_share_basic: float, market_price_per_share: float) -> float:
    """
    Earnings-to-Price (E/P) Ratio (Earnings Yield)
    Formula: Earnings Per Share / Market Price Per Share
    """
    return earnings_per_share_basic / market_price_per_share

def earnings_to_price_ratio_alt(profit_for_the_year: float, market_capitalization: float) -> float:
    """
    Earnings-to-Price (E/P) Ratio - Alternative
    Formula: Net Income / Market Capitalization
    """
    return profit_for_the_year / market_capitalization

def enterprise_value(market_capitalization: float, total_borrowings: float, cash_and_equivalents: float, non_controlling_interest: float=0, preferred_equity: float=0) -> float:
    """
    Enterprise Value (EV)
    Formula: Market Cap + Total Debt + Minority Interest + Preferred Equity - Cash and Cash Equivalents
    """
    return market_capitalization + total_borrowings + non_controlling_interest + preferred_equity - cash_and_equivalents
...
def ev_to_ebit(enterprise_value: float, operating_profit: float) -> float:
    """
...
def tangible_book_value_per_share(tangible_book_value: float, number_of_shares: float) -> float:
    """
    Tangible Book Value Per Share
    Formula: Tangible Book Value / Shares Outstanding
    """
    return tangible_book_value / number_of_shares
...
def price_to_free_cash_flow_alt(market_capitalization: float, free_cash_flow: float) -> float:
    """
    Price-to-Free Cash Flow (P/FCF) - Alternative
    Formula: Market Capitalization / Free Cash Flow
    """
    return market_capitalization / free_cash_flow
